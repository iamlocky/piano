...
public class lightKey1 : MonoBehaviour {
    ...
    void FixedUpdate () {
        if (mainCtrl.target == null)
        {
            ...
            mainCtrl.targetNumber = 1;
        }
        transform.Translate(Vector3.back * mainCtrl.moveSpeed * Time.deltaTime);
        if(transform.position.z<=-5.15)
        {
            mainCtrl.light1.GetComponent<Light>().color = Color.red;
            mainCtrl.light1.SetActive(true);
            Instantiate(miss);
            mainCtrl.missPoints++;
            mainCtrl.finger1miss++;
            Destroy(gameObject);
        }
    }
}
