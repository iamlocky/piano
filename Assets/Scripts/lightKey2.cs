...
public class lightKey2 : MonoBehaviour {
    ...
    void FixedUpdate () {
        if (mainCtrl.target == null)
        {
            ...
            mainCtrl.targetNumber = 2;
        }
        transform.Translate(Vector3.back * mainCtrl.moveSpeed * Time.deltaTime);
        if(transform.position.z<=-5.15)
        {
            mainCtrl.light2.GetComponent<Light>().color = Color.red;
            mainCtrl.light2.SetActive(true);
            Instantiate(miss);
            mainCtrl.missPoints++;
            mainCtrl.finger2miss++;
            Destroy(gameObject);
        }
    }
}
